...
#[Command('options', 'Example of using options in command', options: new Input\Bag\OptionBag([
    ...
    new Input\Definition\Option(
        'port',
        description: 'The port to connect to ( defaults to 1337 )',
    ...
    new Input\Definition\Option(
        'secret',
        description: 'The secret to connect with',
        mode: Input\Definition\Mode::Optional,
    ),
]))]
    ...
    public function run(Input\InputInterface $input, Output\OutputInterface $output): int
    {
        ...
        $rows = [
            ...
            [
                ...
                $port->getAlias(),
                $port->exists() ? 'Yes' : 'No',
                $port->exists() ? $port->getValue() : 'N/A',
            ],
            [
                $user->getName(),
                $user->getAlias(),
                $user->exists() ? 'Yes' : 'No',
                $user->exists() ? $user->getValue() : 'N/A',
            ],
            [
                $secret->getName(),
                $secret->getAlias(),
                $secret->exists() ? 'Yes' : 'No',
                $secret->exists() ? $secret->getValue() : 'N/A',
            ],
        ];
